...
def flash_attention_qk_softmax(
    q_nope: jax.Array,  # [B, bq_sz * H_q, d_nope] -> [8, 128, 512] * [8, 512, 3072] -> [8, 128, 3072]
    q_pe: jax.Array,  # [B, bq_sz * H_q, d_pe]
  ...
  n_q = q_nope.shape[1]  # bq_sz * aligned_num_q_heads
  assert (
      n_q % num_q_heads == 0
  ...
  # SEQ_ALONG_LANE   k_nope: [b, d_nope, s] -> contract q's d against k dim 1
  k_contract = 1
  s_dim = k_nope.shape[-1]
  s_nope = lax.dot(
      q_nope,
      k_nope,
      dimension_numbers=(([2], [k_contract]), ([0], [0])),
...
def chunked_flash_attention(
    q_nope: Any,  # Ref or Array [B, bq_sz * H_q, d_nope]
    q_pe: Any,  # Ref or Array [B, bq_sz * H_q, d_pe]
    k_nope: Any,  # Ref or Array [B, d_nope, S]
    k_pe: Any,  # Ref or Array [B, d_pe, S]
  ...
  Args:
    q_nope: Full query non-positional ref or array [B, bq_sz * H_q, d_nope].
    q_pe: Full query RoPE ref or array [B, bq_sz * H_q, d_pe].
    k_nope: Latent KV cache ref or array (C_kv) of width d_nope.
    k_pe: Decoupled RoPE key cache ref or array (K_pe) of width d_pe.
